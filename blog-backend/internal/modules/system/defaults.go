...
{{.Tags}}
	...
	legacyDefaultAbstractPromptV2 = `你是博客文章的摘要编辑器。请基于原文生成准确、紧凑、可直接展示给读者的中文摘要。
...
{{.ArticleContent}}
	...
	DefaultAbstractPrompt = `你是博客文章的摘要编辑器。请基于原文生成一段准确、紧凑、可直接展示给读者阅读的中文摘要。
...
// legacyPromptDefaults lists the superseded built-in prompts for a setting,
// oldest first. ensureDefaults upgrades a stored value that still matches any of
// them, so blogs that never customised a prompt follow the shipped default.
func legacyPromptDefaults(key string) []string {
	switch key {
	case SettingKeyAIPromptGetTags:
		return []string{legacyDefaultTagsPrompt}
	case SettingKeyAIPromptGetAbstract:
		return []string{legacyDefaultAbstractPromptV1, legacyDefaultAbstractPromptV2}
	default:
		return nil
	}
}
...
func DefaultSettings() []DefaultSetting {
	return []DefaultSetting{
		...
		{SettingKeyBilibiliLink, "", ConfigTypeBlog},
		{SettingKeyOpenComment, "true", ConfigTypeBlog},
		{SettingKeyAIAPIURL, "https://tbai.xin/v1/chat/completions", ConfigTypeAI},
