...
FRONT_TEMP_DIR="$(pwd)/front"
EMBED_DIR="$BACKEND_DIR/internal/frontend/dist"

# 1. 准备目录
rm -rf "$DEPLOY_DIR" "$FRONT_TEMP_DIR" "$EMBED_DIR"
mkdir -p "$DEPLOY_DIR"

# 2. 构建前端
echo "构建前端..."
(cd "$FRONTEND_DIR" && pnpm build)
mv "$FRONTEND_DIR/dist" "$FRONT_TEMP_DIR"

# 3. 嵌入前端到后端
echo "准备后端嵌入前端文件..."
mkdir -p "$EMBED_DIR"
cp -r "$FRONT_TEMP_DIR"/* "$EMBED_DIR/"
...
# 5. 清理中间文件
echo "清理中间文件..."
rm -rf "$FRONT_TEMP_DIR" "$EMBED_DIR"
